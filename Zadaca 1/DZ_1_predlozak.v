...
Proof. intros. simpl. assert(H1: forall x y:list B, AndL (NotL x) (NotL y) = NotL(OrL x y)).
- induction x0,y0.
 -- simpl. reflexivity.
 -- simpl. reflexivity.
 -- simpl. reflexivity.
 -- simpl.  specialize IHx0 with y0. rewrite IHx0. induction a.
    --- simpl. reflexivity.
    ---  simpl. reflexivity.
- specialize H1 with x y. rewrite H1. assert(H2: forall a : list B, NotL (NotL a) = a).
  + intros. induction a.
   ++ simpl. reflexivity.
   ++ simpl. rewrite IHa. induction a. 
    * simpl. reflexivity.
    * simpl. reflexivity.
  + specialize H2 with (OrL x y). apply H2.
Qed.

Lemma ALU_One (n : nat) (x y : list B) :
  length x = n /\ length y = n /\ n <> 0 -> ALU x y I I I I I I = repeat O (pred n) ++ [I].
Proof. intros. destruct H.  destruct H0. simpl. rewrite H. rewrite H0.
Admitted.
